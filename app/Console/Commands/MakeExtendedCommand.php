...
class MakeExtendedCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        if ($createController || $resourceController) {
            $this->createController($name, $resourceController, $createOrion);
        }

        // Erstellen der StageFlow-Struktur
        if ($createOrion) {
            $this->createOrionStructure($name);
        }
    ...
    protected function createOrionStructure($name): void
    {
        $basePath = base_path('orion/Modules/' . $name);
        $modelPath = $basePath . '/Models';
        $repositoryPath = $basePath . '/Repositories';
        $servicePath = $basePath . '/Services';
        $httpPath = $basePath . '/Http/Controllers';
        ...
        if (File::exists($modelFilePath)) {
            ...
            $this->updateNamespace("$modelPath/$modelClass.php", "Orion\\Modules\\$name\\Models");
        }

        // Repository erstellen
        $repositoryClass = $name . 'Repository';
        $this->createFileFromStub('Repository', "$repositoryPath/$repositoryClass.php", [
            '{{namespace}}' => "Orion\\Modules\\$name\\Repositories",
        ...
        $this->createFileFromStub('Service', "$servicePath/$serviceClass.php", [
            '{{namespace}}' => "Orion\\Modules\\$name\\Services",
            '{{class}}' => $serviceClass,
            '{{repository}}' => $repositoryClass,
            '{{repositoryVariable}}' => lcfirst($repositoryClass),
            '{{repositoryNamespace}}' => "Orion\\Modules\\$name\\Repositories\\$repositoryClass",
        ]);
        ...
        if (File::exists($controllerFilePath)) {
            File::move($controllerFilePath, "$httpPath/$controllerName.php");
            $this->updateNamespace("$httpPath/$controllerName.php", "Orion\\Modules\\$name\\Http\\Controllers");
        }

        $this->info("Orion structure created for $name.");
    }
    ...
    protected function linkServiceInController($name, $controllerName): void
    {
        ...
        $serviceClass = "{$name}Service";
        $serviceNamespace = "Orion\\Modules\\$name\\Services\\$serviceClass";
