...
<template>
  <div class="flex items-center rounded-bl-xl border-solid border-t border-r-2 border-primary bg-primary-dark"
    ...
    <div
      ...
      :class="{ 'bg-red-500 cursor-pointer': isInProgress && isHovered }" @mouseenter="isHovered = true"
      ...
      <svg v-if="isHovered && isInProgress" xmlns="http://www.w3.org/2000/svg"
        class="text-white relative z-10 transition-all duration-300" width="24" height="24" viewBox="0 0 24 24">
        <path fill="currentColor"
      ...
      <div class="absolute top-0 transition left-0 h-11 w-full bg-secondary rounded-bl-xl progress-bar"
        :class="{ 'bg-red-500': isInProgress && isHovered }"
