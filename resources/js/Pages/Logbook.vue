...
// Mapping für Schiffsdaten (Typ, Bild, etc.) nach Name
...
<template>
  <AppLayout title="logbook">
    ...
    <div>
      <div class="space-y-4">
        <div v-for="log in props.logs" :key="log.id" class="bg-base rounded-xl border-primary border-4 border-solid content_card">
          <button
            ...
            @click="selectedLog && selectedLog.id === log.id ? selectedLog = null : showDetails(log)"
          >
            ...
            <div v-if="selectedLog && selectedLog.id === log.id" class="px-4 pb-4 pt-2">
              ...
              <div v-if="selectedLog.plundered_resources && Object.keys(selectedLog.plundered_resources).length">
                <h3 class="text-lg font-bold mb-2 text-light">
                  {{ userId === selectedLog.defender_id ? 'Ressources lost' : 'Ressources gained' }}
                </h3>
                <ul class="flex flex-wrap gap-4 text-light">
                  <li v-for="(amount, resourceId) in selectedLog.plundered_resources" :key="resourceId" class="flex items-center gap-2 bg-primary/10 rounded px-3 py-2">
                    ...
                    <span :class="[userId === selectedLog.defender_id ? 'font-bold text-red-400' : 'font-bold text-light']">{{ numberFormat(amount) }}</span>
                  </li>
                </ul>
              </div>
            </div>
